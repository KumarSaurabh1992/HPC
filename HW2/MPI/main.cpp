...
bin_particles(std::vector<particle_t> &local_particles,  grid_t *grid, double start_proc_boundary,
              double end_proc_boundary, int NumGrid, double dx) {
    ...
    for (int i = 0; i < local_particles.size(); i++) {
        ...
        idx = static_cast<unsigned int>(x * dx);
        idy = static_cast<unsigned int>(local_particles[i].y * dx);
        id = (idx * NumGrid + idy);
        grid[id].id_[grid[id].count_] = counter;
...
//        std::cout << "Step =  " << step << " Rank = " <<rank << " Local Size = " <<  local_part << "\n";
        ...
        packBuffers(local_particles, send_up, send_down, exchangeBoundaryUp, exchangeBoundaryDown);
        MPI_Request request[2];
        MPI_Status status[2];
        ...
        clear_bins(grid, num_row * NumGrid);
        bin_particles(local_particles,grid,start_proc_boundary,end_proc_boundary,NumGrid,dx);
